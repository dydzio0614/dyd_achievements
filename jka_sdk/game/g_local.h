...
struct gentity_s {
	...
	void		(*freeEntity)(gentity_t *self);
	void		(*examine)(gentity_t *self, gentity_t *activator);
	char		*examineStr;
...
typedef struct iObjectFields_s{
char *name;
qboolean noCombine;
qboolean noDelete;
qboolean noGive;
...
// Lugormod Account Data Structure
typedef struct {
char *username;
...
struct gclient_s {
	...
	struct{
		...
		int shieldTime; // lmd_playereffect
	}Lmd;
...
struct profSkill_s{
	...
	int (*getValue)(Account_t* acc, profSkill_t *skill);
	qboolean (*canSetValue)(Account_t* acc, profSkill_t *skill, int value);
	qboolean (*setValue)(Account_t* acc, profSkill_t *skill, int value);
...
typedef struct accList_s
{
	unsigned int count;
	Account_t** list;
} accList_t;

typedef struct authFile_s authFile_t;
typedef struct authCmdEntry_s authCmdEntry_t;
...
typedef struct authFileDataList_s{
unsigned int count;
authFileData_t *list;
}authFileDataList_t;
...
qboolean autoGenerated; //True if this was created by cmdleveldefs, false if it is a named file.
};
...
int iArg;
qboolean isAdmin;
int level;	 //allowed at authentication level 
int disable; //g_cmdDisable bitmask
int gametype; //disabled in gametype (bitmask)
int profession; //usable by profession (0 = all)
qboolean locked; // Unable to be overrode by cmdleveldef

// Set at run time
cmdCategory_t *category; // The parent command category.
};

struct authCmdEntry_s{
cmdEntry_t *cmd;
int report; // Report to admins this level
int log; // Never finished as far as I can remember; was going to log commands to a file.
int restr; // Disable unless an admin of this level is on
int cost; // Cost this many command points to use
};

typedef struct authList_s {
	int count;
	authFile_t* authList;
} authList_t;
